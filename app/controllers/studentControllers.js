...
function registerValidate(insertnewdata) {
  const schema = {
    studentName: Joi.string().required().error(()=>{
      return{
        message:'Please Enter a Student Name'
      }
    }).min(3).max(30),
    dateOfBirth: Joi.string().required().min(10).max(11).error(()=>{
      return{
        message:'Please Enter a Date of Birth '
      }
    }),
    email: Joi.string().email({ tlds: { allow: false } }).required().error(()=>{
      return{
        message:"Please Enter a Email ID"
      }
    }).regex(/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/).error(()=>{
      return{
        message:"Please Enter a Valid Email Address"   
      }
    }),
    password: Joi.string().required().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$@!%&*?])[A-Za-z\d#$@!%&*?]{8,30}$/).error(() => {
      return {
        message: 'Your Password Should be 8 charcters It should have atleast one upper case charater, one lower case charater, one number and one special symbol (* and #)',
      };
    }),
    parentName: Joi.string().required().error(()=>{
      return{
        message:'Please Enter a Parent  Name'
      }
    }),
    parentRelation: Joi.string().required().error(()=>{
      return{
        message:'Please Enter a Parent Relation'
      }
    }),
    studentMobileNumber: Joi.string().required().error(()=>{
      return{
        message:'Please Enter a Student Mobile Number '
      }
    }).regex(/^(?:(?:\+|0{0,2})91(\s*[\-]\s*)?|[0]?)?[6789]\d{9}$/).error(() => {
      return {
        message: 'Please Enter Student Mobile Number 10 Digit Mobile Number',
      };
    }),
    parentMobileNumber: Joi.string().required().error(()=>{
      return{
    ...
    course: Joi.string().required().error(()=>{
      return{
    ...
    enrollNo: Joi.string().required().error(()=>{
      return{
        message:'Please Enter a Enroll Number '
      }
    }),
    enrollDate: Joi.string().required().error(()=>{
      return{
        message:'Please Enter a Enroll Date '
      }
    }),
    entry: Joi.string().required().error(()=>{
      return{
    ...
    admissionDate: Joi.string().required().error(()=>{
      return{
...
function changePasswordValidate(validData) {
  const schema = {
    ...
    newPassword: Joi.string().required().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$@!%&*?])[A-Za-z\d#$@!%&*?]{8,30}$/).error(() => {
      return {
        message: 'Your Password Should be 8 charcters It should have atleast one upper case charater, one lower case charater, one number and one special symbol (* and #)',
      };
    })
...
function updateProfileValidate(validData) {
  const schema = {
    email: Joi.string().lowercase().regex(/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i).error(()=>{
      return{
        message:"Please Enter a Valid Email Address"
      }
    })
